...
async getPlatformRevenueReport(req, res) {
  try {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SUBSCRIPTION PAYMENT REVENUE (from votteryy_payments)
    // This is the actual subscription fee revenue paid by users
    // ─────────────────────────────────────────────────────────────────────────
    const subscriptionPaymentRevenueResult = await pool.query(`
      SELECT
        COUNT(*) as total_subscription_payments,
        COUNT(*) FILTER (WHERE status = 'succeeded' OR status = 'completed' OR status = 'success') as successful_payments,
        COALESCE(SUM(CASE WHEN status IN ('succeeded', 'completed', 'success') THEN amount ELSE 0 END), 0) as total_subscription_revenue,
        COALESCE(SUM(CASE WHEN status IN ('succeeded', 'completed', 'success') AND created_at >= NOW() - INTERVAL '${periodDays} days' THEN amount ELSE 0 END), 0) as subscription_revenue_this_period,
        COALESCE(SUM(CASE WHEN status IN ('succeeded', 'completed', 'success') AND created_at >= NOW() - INTERVAL '30 days' THEN amount ELSE 0 END), 0) as subscription_revenue_30d,
        COALESCE(SUM(CASE WHEN status IN ('succeeded', 'completed', 'success') AND created_at >= NOW() - INTERVAL '7 days' THEN amount ELSE 0 END), 0) as subscription_revenue_7d,
        COALESCE(SUM(CASE WHEN status IN ('succeeded', 'completed', 'success') AND created_at >= NOW() - INTERVAL '1 day' THEN amount ELSE 0 END), 0) as subscription_revenue_today
    ...
    // COMBINED TOTALS
    // ─────────────────────────────────────────────────────────────────────────
    // CHANGED: Now using subscriptionPaymentRevenue from votteryy_payments table
    const subscriptionPaymentRevenue = parseFloat(subscriptionPaymentRevenueResult.rows[0]?.total_subscription_revenue || 0);
    const platformFeeRevenue = parseFloat(platformFeeResult.rows[0]?.total_platform_fee_revenue || 0);
    const totalPlatformRevenue = subscriptionPaymentRevenue + platformFeeRevenue;
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          subscription_percentage: totalPlatformRevenue > 0 ? ((subscriptionPaymentRevenue / totalPlatformRevenue) * 100).toFixed(2) : 0,
        ...
        subscriptions: {
          stats: {
            ...subscriptionStatsResult.rows[0],
            // Add subscription payment stats
            total_subscription_revenue: subscriptionPaymentRevenue,
            subscription_revenue_this_period: parseFloat(subscriptionPaymentRevenueResult.rows[0]?.subscription_revenue_this_period || 0),
            subscription_revenue_30d: parseFloat(subscriptionPaymentRevenueResult.rows[0]?.subscription_revenue_30d || 0),
            subscription_revenue_7d: parseFloat(subscriptionPaymentRevenueResult.rows[0]?.subscription_revenue_7d || 0),
            subscription_revenue_today: parseFloat(subscriptionPaymentRevenueResult.rows[0]?.subscription_revenue_today || 0)
          },
