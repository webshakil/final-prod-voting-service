...
// // ========================================
// // GET LIVE RESULTS - SUPPORTS BOTH NORMAL & ANONYMOUS VOTES
// // ========================================
// export const getLiveResults = async (req, res) => {
//   const client = await pool.connect();
...
//     console.log(`📊 Fetching live results for election: ${electionId}`);
...
//     console.log(`🔐 Election is ${isAnonymous ? 'ANONYMOUS' : 'NORMAL'}`);
...
//     // 2. Get questions with vote counts - HANDLE BOTH TABLES
//     let questionsQuery;
    
//     if (isAnonymous) {
//       // ⭐ FOR ANONYMOUS ELECTIONS: Query votteryyy_anonymous_votes
//       questionsQuery = `
//         SELECT 
//           q.id,
//           q.question_text,
...
//                 (
//                   SELECT COUNT(*)
//                   FROM votteryyy_anonymous_votes v
//                   WHERE v.election_id = $1
//                   AND v.answers ? q.id::text
//                   AND (
//                     -- For plurality/single choice
//                     (v.answers->q.id::text)::text = o.id::text
//                     OR
//                     -- For approval voting (array contains option id)
//                     v.answers->q.id::text @> o.id::text::jsonb
//                     OR
//                     -- For ranked choice (object contains option id as key)
//                     v.answers->q.id::text ? o.id::text
//                   )
//                 ),
//                 0
//               )
...
//       `;
//     } else {
//       // ⭐ FOR NORMAL ELECTIONS: Query votteryy_votes
//       questionsQuery = `
//         SELECT 
//           q.id,
//           q.question_text,
...
//     const questionsResult = await client.query(questionsQuery, [electionId]);

//     // 3. Get total votes from appropriate table
//     let totalVotesQuery;
...
//     const totalVotesResult = await client.query(totalVotesQuery, [electionId]);
//     const totalVotes = parseInt(totalVotesResult.rows[0].total);

//     console.log(`✅ Total votes counted: ${totalVotes} (from ${isAnonymous ? 'anonymous' : 'normal'} table)`);

//     // 4. Calculate percentages for each option
//     const questions = questionsResult.rows.map(question => {
//       const questionTotalVotes = question.options?.reduce(
...
//         percentage: questionTotalVotes > 0 
//           ? ((option.vote_count / questionTotalVotes) * 100).toFixed(2)
//           : '0.00',
//       }));
...
//     console.log(`✅ Live results retrieved: ${totalVotes} total votes`);
//     console.log(`📊 Results breakdown:`, questions.map(q => ({
//       question: q.question_text,
//       total: q.total_votes,
//       options: q.options.map(o => `${o.option_text}: ${o.vote_count}`)
//     })));
...
// //     const electionQuery = `
// //       SELECT 
// //         id, title, voting_type, show_live_results, status
// //       FROM votteryyy_elections
// //       WHERE id = $1
// //     `;
...
// //     // 2. Get questions with vote counts
...
//       const receiptId = generateReceiptId();
//       const verificationCode = generateVerificationCode();
//       const voteToken = generateVoteToken(); //  NEW: Generate vote token
...
//         success: true,
//         anonymous: true,
//         voteToken,  //  User needs this to verify their vote
//         voteHash,
//         receiptId,
//         verificationCode,
//         message: 'Anonymous vote cast successfully. Save your vote token to verify your vote later.',
...
//             receiptId,
//             verificationCode,
//             false, // ⭐ anonymous = false for normal voting
//             req.ip || req.connection.remoteAddress,
//             req.headers['user-agent'],
//           ]
//         );
...
// /**
//  * Helper function to get live results data (reusable)
//  */
// /**
//  * Helper function to get live results data (reusable for Socket.IO)
//  * ⭐ UPDATED: Handles both normal and anonymous votes
//  */
// export const getLiveResults = async (req, res) => {
//   const client = await pool.connect();
...
//         // ⭐ FIXED: Count votes for this specific option
//         // This query handles plurality, approval, and ranked-choice voting
//         const voteCountQuery = `
//           SELECT COUNT(DISTINCT v.id) as count
//           FROM ${votesTableName} v
//           WHERE v.election_id = $1
...
//       votingType: election.voting_type,
//       isAnonymous: isAnonymous,
//       totalVotes: grandTotalVotes, // ⭐ Use calculated total, not DB count
//       questions,
//       lastUpdated: new Date().toISOString(),
//     };
...
// //        FROM votteryyy_election_questions q
// //        LEFT JOIN votteryy_question_options o ON q.id = o.question_id
// //        LEFT JOIN votteryy_vote_answers va ON o.id = va.option_id
// //        LEFT JOIN votteryy_votes v ON va.vote_id = v.id AND v.status = 'valid'
// //        WHERE q.election_id = $1
...
//     // ✅ ADD THIS: Get minimum required percentage to auto-complete if needed
//     const electionResult = await pool.query(
//       `SELECT minimum_watch_percentage FROM votteryyy_elections WHERE id = $1`,
//       [electionId]
//     );
...
//     // ✅ ADD THIS: Auto-complete if threshold met (fallback if frontend doesn't send it)
//     const isCompleted = completed || (watchPercentage >= minPercentage);
...
//       lastPositionInt,
//       totalDurationInt,
//       completed: isCompleted, // ✅ CHANGED: Use calculated value
//       minPercentage, // ✅ ADD THIS to logging
//     });
...
//       success: true,
//       watchPercentage: watchPercentageInt,
//       completed: isCompleted, // ✅ CHANGED: Return calculated value
//       data: result.rows[0],
//     });
...
//   getVotingHistory
// };
//last workable code only to implement anonymous and abstentia voting above code
// // src/controllers/voting.controller.js
// // ✨ COMPLETE VERSION - All vote-related operations - FULLY FIXED
// import pool from '../config/database.js';
// import crypto from 'crypto';
...
//     // ✅ SOCKET.IO INTEGRATION - Emit real-time updates
//     setImmediate(async () => {
//       try {
//         const { emitVoteCast, emitLiveResultsUpdate } = await import('../socket/votingSocket.js');
...
//         emitVoteCast(electionId, {
//           questionId: Object.keys(answers)[0], // First question ID
//           timestamp: new Date().toISOString(),
//         });

//         // Fetch and emit updated live results
//         const liveResultsResponse = await getLiveResultsData(electionId);
//         if (liveResultsResponse) {
//           emitLiveResultsUpdate(electionId, liveResultsResponse);
//         }
...
//   getVotingHistory
// };
//this is the last workbale code. to implement socket.io above code
// // src/controllers/voting.controller.js
// // ✨ COMPLETE VERSION - All vote-related operations - FULLY FIXED
// import pool from '../config/database.js';
// import crypto from 'crypto';
