...
console.log('🔍 PARTICIPATION CHECK:', {
 ...
 console.log('🗳️ HAS VOTED RESULT:', hasVoted);
    ...
    if (hasVoted) {
      if (election.anonymous_voting_enabled) {
        // Anonymous vote - get via voting_session_id
        const anonVote = await pool.query(
          `SELECT voting_id, vote_hash, receipt_id FROM votteryyy_anonymous_votes 
           WHERE voting_session_id = $1`,
          [participationCheck.rows[0].voting_session_id]
        );
        voteDetails = anonVote.rows[0] || null;
      } else {
        // Normal vote - get directly
        const normalVote = await pool.query(
          `SELECT voting_id, vote_hash, receipt_id FROM votteryy_votes 
